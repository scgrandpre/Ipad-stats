...
- (NSString*) typeForClass:(Class) class {
    ...
    while (true) {
        NSString* part = @"";
        [scanner scanCharactersFromSet: [NSCharacterSet uppercaseLetterCharacterSet] intoString: &part];
        type = [type stringByAppendingString: [part lowercaseString]];
        [scanner scanCharactersFromSet: [NSCharacterSet lowercaseLetterCharacterSet] intoString: &part];
...
- (void) GetSerializable: (Class<Serializable>) class withId: (NSString*) id  callback: (void (^)(NSObject<Serializable>* object)) callback {
    ...
    GetJSON(BackendURL([NSString stringWithFormat: @"%@/%@", type, id]), ^(NSDictionary *dict, NSError* error) {
        if (dict == Nil ) {
            callback(Nil);
        } else {
...
- (void) GetRangeSerializable:(Class<Serializable>) class withStart:(int) start end:(int) end callback: (void (^)(NSArray* object)) callback {
    ...
    GetJSON(BackendURL([NSString stringWithFormat: @"%@s/range/%d:%d", type, start, end]),
...
- (void) DeleteSerializable:(NSObject<Serializable>*) object callback: (void (^)(NSURLResponse* response, NSData* result, NSError* error)) callback {
    ...
    NSURL* url = BackendURL([NSString stringWithFormat:@"%@/%@", type, object.id]);
...
- (UIColor*) DeserializeUIColor:(NSDictionary*) colorDict {
    return [[UIColor alloc] initWithRed: [[colorDict objectForKey: @"red"  ] floatValue]
